...
void findnext()           // nowvec stroged the previous number array
{
    if (nowsize == level)              // if nowvec already had enough number(already went through enough recursive)
    {
    ...
    for (auto i:allowedvec)                                        //find new number to pushback
        ...
        {
            nowsize++;
            nowvec.push_back(i);                                        //pushback the new number
            findnext();
            nowvec.pop_back();
            nowsize--;                                           // recursive
        }
