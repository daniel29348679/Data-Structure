...
int main()
{
    ...
    while(k != 0)
    {
        ...
        if(k == 4)
        {
            ...
            cout << "\nInput M: ";
            cin >> level;                  //M numbers means M recursive level
            while(level < 1 || level > n)  // make sure M is in [1,N]
            {
                cout << "\n### It is NOT in [1,n] ###\n\nInput a number: ";
                cin >> level;
            }
            ...
            allowedvec.clear();               //reset allowedvec
            for(int i = 0; i < n; i++)
                allowedvec.push_back(i + 1);  //put 1,2,3,4,5,6,7,8,9,,,,n into allowedvec
